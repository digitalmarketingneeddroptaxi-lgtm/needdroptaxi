...
metadata = {
    ...
    './outstation-taxi-service/index.html': {
        ...
        'desc': 'Reliable outstation taxi service from Chennai and across Tamil Nadu. One-way drop taxis and round trips starting at Rs 13/km with no hidden charges.'
    },
    './blog/index.html': {
    ...
    './blog/top-10-tourist-attractions-to-visit-in-chennai-with-need-drop-taxi/index.html': {
        ...
        'desc': "Discover top 10 Chennai attractions, from Marina Beach to Kapaleeshwarar Temple. Enjoy flexible sightseeing with Need Drop Taxi local hourly rentals."
    },
...
for file_path, data in metadata.items():
    ...
    # First find existing tag:
    m_tag = re.search(r'<meta\s+[^>]*name=[\'"]description[\'"][^>]*>', content)
    if m_tag:
        content = content[:m_tag.start()] + f'<meta name="description" content="{new_desc}">' + content[m_tag.end():]
    else:
        # check reverse order
        m_tag2 = re.search(r'<meta\s+[^>]*content=[\'"][^\'"]*[\'"][^>]*name=[\'"]description[\'"][^>]*>', content)
        if m_tag2:
            content = content[:m_tag2.start()] + f'<meta name="description" content="{new_desc}">' + content[m_tag2.end():]

    # 3. Update OG & Twitter
    for og_prop in ['og:title', 'twitter:title']:
        m_og = re.search(r'<meta\s+[^>]*property=[\'"]' + og_prop + r'[\'"][^>]*>', content)
        if not m_og:
            m_og = re.search(r'<meta\s+[^>]*name=[\'"]' + og_prop + r'[\'"][^>]*>', content)
        if m_og:
            attr = 'property' if 'og:' in og_prop else 'name'
            content = content[:m_og.start()] + f'<meta {attr}="{og_prop}" content="{new_title}">' + content[m_og.end():]

    for og_prop in ['og:description', 'twitter:description']:
        m_og = re.search(r'<meta\s+[^>]*property=[\'"]' + og_prop + r'[\'"][^>]*>', content)
        if not m_og:
            m_og = re.search(r'<meta\s+[^>]*name=[\'"]' + og_prop + r'[\'"][^>]*>', content)
        if m_og:
            attr = 'property' if 'og:' in og_prop else 'name'
            content = content[:m_og.start()] + f'<meta {attr}="{og_prop}" content="{new_desc}">' + content[m_og.end():]

    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(content)

print("Meta description clean update finished.")
